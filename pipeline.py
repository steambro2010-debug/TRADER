...
class MarketIntelligencePipeline:
    ...
    def run(self, input_csv: str, output_json: str) -> dict:
        ...
        labeled = triple_barrier_labels(
            ...
            min_no_trade_ratio=float(cfg["training"]["labeling"].get("min_no_trade_ratio", 0.05)),
            enforce_three_classes=bool(cfg["training"]["labeling"].get("enforce_three_classes", True)),
        )
        ...
        wf_losses = []
        n_splits = int(cfg["training"].get("walk_forward_splits", 3))
        n_splits = max(2, min(n_splits, max(2, len(train_df) // 200)))
        for fold_idx, (tr_idx, te_idx) in enumerate(walk_forward_splits(train_df, n_splits=n_splits), start=1):
